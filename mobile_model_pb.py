...
img = tf.placeholder(tf.float32, shape=[BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, 3], name='I')
img = tf.squeeze(img, 0)
img = tf.cast(img, tf.float32) * (1. / 255) - 0.5
img = tf.image.per_image_standardization(img)
image = tf.expand_dims(img, 0)
with slim.arg_scope(mobilenet_v2_arg_scope(is_training=False)):
    logits_eval, end_points_eval = mobilenet_v2(image, num_classes=3, is_training=False)
logits_eval = tf.identity(logits_eval)
...
with tf.Session() as sess:
    ...
    tf.train.write_graph(sess.graph_def, SAVE_DIR, 'tfdroid_image.pbtxt')
    saver.save(sess, os.path.join(SAVE_DIR, 'tfdroid_image.ckpt'))
